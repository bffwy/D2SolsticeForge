    ...
    if r.status_code == 200:
        try:
            ...
            for character_id, character in character_data.items():
                print(f"get_characters: {character_id}: {character}")
...
def story_item_data(item_info):
    # print("story_item_data: ", item_info)
    if "itemInstanceId" in item_info:
        # 只存没有 itemInstanceId的数据
        return
    item_Hash = item_info["itemHash"]
    quantity = item_info.get("quantity", 0)
    if quantity == 0:
        return
    profile_item_quantity[item_Hash] = quantity


def get_profile_inventory():
    """获取 inventory 信息"""
    api_path = f"/Destiny2/{config.dim_settings.membership_type}/Profile/{config.dim_settings.destiny_membership_id}/?components=102"
    r = requests.get(CONST.URL_BASE + api_path, headers=COMMON_HEADERS)
    if r.status_code == 200:
        try:
            data = r.json()
            # print("get_profile_inventory: ", data)
            with open("./profile_inventory.json", "w", encoding="utf-8") as f:
                json.dump(data, f, ensure_ascii=False)
            items = data["Response"]["profileInventory"]["data"]["items"]
            for item in items:
                story_item_data(item)
        except Exception as e:
            pass
...
def check_before_move(item):
    ...
    if not item_bucket_type_hash or int(item_bucket_type_hash) not in CONST.ArmorBucketTypeHash:
...
def get_item_quantity(item_hash):
    return profile_item_quantity.get(item_hash, 0)
