...
from PyQt5.QtGui import QFont, QPixmap, QTextCharFormat, QTextCursor, QColor, QEnterEvent, QTextBlockFormat
...
init_line_num = 10
init_line_height = 35
init_user_color = "#00FF00"
init_com_color = "#E0FFFF"
init_font = "宋体"


ui_path = path_helper.get_resource("my_window/ui/DisplayWindow.ui")
# init_background = get_real_path("my_window/config/background_10.png", "resource")
init_background = ""
...
class DisplayWindow(QDockWidget):
    def __init__(self, parent=None):
        ...
        # 显示样式
        self.Userfont = QColor(init_user_color)
        self.Comfont = QColor(init_com_color)
        # 初始化
        self.Ui_init()
        ...
        screen_rect = desktop.screenGeometry()  # 获取屏幕大小
        self.move(screen_rect.width() - self.width(), 0)  # 移动窗口
    ...
    def Ui_init(self):
        ...
        self.textEdit.setMaximumBlockCount(init_line_num)
        font = QFont(init_font)
        font.setWordSpacing(20)
        self.textEdit.setFont(font)
        self.textEdit.setStyleSheet(
            f"font-size:{init_size}px;border: none; background-color: transparent; font-weight: bold;"
        )
        ...
        # 设置行高，但不明显，可有可无
        self.BlockFormat.setLineHeight(init_line_height, QTextBlockFormat.FixedHeight)
    ...
    def resizeEvent(self, QResizeEvent):
        ...
        ran = 30
        self._right_rect = [
            QPoint(x, y) for x in range(self.width() - ran, self.width()) for y in range(1, self.height() - ran)
        ]
        self._bottom_rect = [
            QPoint(x, y) for x in range(1, self.width() - ran) for y in range(self.height() - ran, self.height())
        ]
        self._corner_rect = [
            QPoint(x, y)
            for x in range(self.width() - ran, self.width())
